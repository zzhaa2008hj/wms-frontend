...
export class Approve {
  ...
  async activate({ id }) {
    ...
    this.cargoMortgage.cancelPledgeTimeStr = moment(this.cargoMortgage.cancelPledgeTime).format('YYYY-MM-DD HH:mm:ss');
  ...
  async approve(status) {
    let confirmed = await this.dialogService.confirm({ title: "审批确认", message: '确认执行该操作？' });
    if (!confirmed) return;
    try {
      await this.cargoMortgageService.approveCargoMortgage(this.cargoMortgage.id, status);
      await this.dialogService.alert({ title: "", message: '审批成功' });
